...
class ICPRegistration:
    ...
    def bar_tw_icp_registration(self, source, target, init_transform=np.eye(4),
        ...
        # 将源点云转换为 Open3D 点云格式
        transformed_source_pcd = o3d.geometry.PointCloud()
        transformed_source_pcd.points = o3d.utility.Vector3dVector(src_points)

        # 计算均方误差 (RMSE)
        rmse = calculate_rmse(np.asarray(transformed_source_pcd.points), tgt_points)

        # 计算重叠率
        overlap_ratio = calculate_overlap_ratio(np.asarray(transformed_source_pcd.points), tgt_points)


        return current_transform, transformed_source_pcd, overlap_ratio, rmse, len(valid_pairs)
