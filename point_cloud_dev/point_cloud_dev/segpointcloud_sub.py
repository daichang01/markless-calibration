...
class PointCloud2Subscriber(Node):
    ...
    def timer_callback(self):
        # 定时保存点云数据
        if self.points:
            # filename = f"src/markless-calibration/pcd/origin_data/ori{self.save_index}.txt"
            filename = f"src/markless-calibration/pcd/01origin_data/seg{self.save_index}.txt"
