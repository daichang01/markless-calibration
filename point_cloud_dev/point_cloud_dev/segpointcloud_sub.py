...
class PointCloud2Subscriber(Node):
    ...
    def lowlistener_callback(self, msg):
        # 解析点云数据
        point_list = list(pc2.read_points_list(msg, field_names=("x", "y", "z", "rgb"), skip_nans=True))
        self.get_logger().info(f'Received {len(point_list)} points in lowlistener_callback')
    ...
    def uptimer_callback(self):
        ...
        if self.uppoints:
            ...
            self.get_logger().info(f'已保存 {len(self.uppoints)} 个点到文件 {filename}')
            # 清空点列表和颜色列表以准备下一个点云
            self.uppoints = []
            self.upcolors = []
    ...
    def lowtimer_callback(self):
        ...
        if self.lowpoints:
            ...
            self.save_point_cloud_to_txt(filename)
            self.get_logger().info(f'已保存 {len(self.lowpoints)} 个点到文件 {filename}')
            # 清空点列表和颜色列表以准备下一个点云
            self.lowpoints = []
            self.lowcolors = []
