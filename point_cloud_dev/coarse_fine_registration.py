...
################################################  Registration Pipeline #############################################
    ...
    ####################  icp精配准  ###################################################################################################
        ...
        # fine_transformation, fine_transformed_source, overlap_ratio, rmse, num_valid_pairs = self.icp_registrator.bar_tw_icp_registration \
        #     (transformed_source_cloud, self.target,coarse_transformation)

        # end_time_bw = time.time()
        # icp_time = start_time_bw - end_time_bw
        # # print(f"icp soft精配准后的变换矩阵：{fine_transformation}")
        # self.get_logger().info(f"双向自适应鲁棒加权ICP精配准后的评估结果：")
        # self.get_logger().info(f"双向自适应鲁棒加权ICP  RMSE: {rmse}")
        # self.get_logger().info(f"双向自适应鲁棒加权ICP  overlap_ratio: {overlap_ratio}")
        # self.get_logger().info(f"双向自适应鲁棒加权ICP  精配准耗时: {icp_time} 秒,有效点对数量: {num_valid_pairs}")
